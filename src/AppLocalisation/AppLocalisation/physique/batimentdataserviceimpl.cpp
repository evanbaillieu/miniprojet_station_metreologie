...
Batiment Physique::BatimentDataServiceImpl::add(Batiment batiment)
{
    ...
    if((execute = query.exec())==true) { // Si la requête s'est éxécuté, on récupère le dernier id inséré dans la bdd
...
{
    int count = 0;
    query.prepare("SELECT COUNT(*) FROM batiment"); // Récupère le nombre total de batiment
    if(query.exec()) {
        if(query.next()){
...
QList<QString> Physique::BatimentDataServiceImpl::getListNomBatiment(int debut, int parPage)
{
    ...
    query.prepare("SELECT nomBatiment FROM batiment WHERE nombatiment != '' ORDER BY nomBatiment ASC LIMIT :debut,:parPage"); // Récupère toutes les valeurs non vide enregistrées dans la bdd correspondant à la colonne nomBatiment
    query.bindValue(":debut", debut);
    query.bindValue(":parPage", parPage);
    if(query.exec()) {
        while(query.next()){
